...
export default function RolesPermissions() {
  ...
  // Fetch roles from API - only if user has permission - optimized query
  ...
  // Fetch permissions for selected role - optimized query
  ...
  // Check if all permissions are checked (globally) - only for visible permissions - memoized
  const areAllPermissionsChecked = useCallback((roleId: number) => {
    const permissions = rolePermissions[roleId] || [];
  ...
  // Check if all permissions in a module are checked - memoized
  const areAllModulePermissionsChecked = useCallback((roleId: number, moduleKey: string) => {
    const permissions = rolePermissions[roleId] || [];
    const localPerms = localPermissions[roleId] || {};
  ...
  // Check if some (but not all) permissions are checked (for indeterminate state) - only for visible permissions - memoized
  const areSomePermissionsChecked = useCallback((roleId: number) => {
    const permissions = rolePermissions[roleId] || [];
    const localPerms = localPermissions[roleId] || {};
    // Filter out excluded modules
    const excludedModules = ['My Devices', 'Prompts', 'Roles', 'Settings'];
  ...
  // Check if some (but not all) permissions in a module are checked - memoized
  const areSomeModulePermissionsChecked = useCallback((roleId: number, moduleKey: string) => {
    const permissions = rolePermissions[roleId] || [];
    const localPerms = localPermissions[roleId] || {};
    // Handle both regular modules and IT Asset Management sub-menus
    const [module, menuName] = moduleKey.split('::');
  ...
  // Toggle all permissions (globally) - only for visible permissions - memoized
  const handleToggleAllPermissions = useCallback((roleId: number, checked: boolean) => {
    const permissions = rolePermissions[roleId] || [];
    const localPerms = localPermissions[roleId] || {};
    // Filter out excluded modules
    const excludedModules = ['My Devices', 'Prompts', 'Roles', 'Settings'];
  ...
  // Toggle all permissions in a module - memoized
  const handleToggleModulePermissions = useCallback((roleId: number, moduleKey: string, checked: boolean) => {
    const permissions = rolePermissions[roleId] || [];
    // Handle both regular modules and IT Asset Management sub-menus
    const [module, menuName] = moduleKey.split('::');
    let modulePerms: any[];
  ...
  // Memoized handlers for edit actions
  const handleEditClick = useCallback((e: React.MouseEvent, roleId: number) => {
    e.stopPropagation();
    handleStartEdit(roleId);
  }, [handleStartEdit]);

  const handleCancelClick = useCallback((e: React.MouseEvent, roleId: number) => {
    e.stopPropagation();
    handleCancelEdit(roleId);
  }, [handleCancelEdit]);

  const handleSaveClick = useCallback((e: React.MouseEvent, roleId: number) => {
    e.stopPropagation();
    handleSavePermissions(roleId);
  }, [handleSavePermissions]);

  // Memoized handler for stop propagation
  const handleStopPropagation = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  }, []);

  // Memoized handlers for checkbox changes
  const handleGlobalCheckAll = useCallback((roleId: number) => {
    return (checked: boolean) => {
      handleToggleAllPermissions(roleId, checked);
    };
  }, [handleToggleAllPermissions]);

  const handleModuleCheckAll = useCallback((roleId: number, moduleKey: string) => {
    return (checked: boolean) => {
      handleToggleModulePermissions(roleId, moduleKey, checked);
    };
  }, [handleToggleModulePermissions]);

  const handlePermissionCheck = useCallback((roleId: number, permissionId: number) => {
    return (checked: boolean) => {
      handlePermissionToggle(roleId, permissionId, checked);
    };
          ...
          <div className="space-y-4">
            {roles.map((role: any) => (
              ...
              >
                ...
                {selectedRole === role.id && (
                  <div className="mt-5 pt-5 border-t border-border/50" onClick={handleStopPropagation}>
                      ...
                      {isSuperAdmin && role.name !== 'Super Admin' && editingPermissions[role.id] && (
                        <div className="flex gap-2">
                          <Button 
                            variant="outline" 
                            size="sm"
                            onClick={(e) => handleCancelClick(e, role.id)}
                          ...
                          <Button 
                            ...
                            onClick={(e) => handleSaveClick(e, role.id)}
                            disabled={updatePermissionsMutation.isPending}
                          >
                              ...
                              {isEditing && (
                                <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-xl border-2 border-primary/20 shadow-sm">
                                  <Checkbox
                                    ...
                                    onCheckedChange={handleGlobalCheckAll(role.id)}
                                    disabled={updatePermissionsMutation.isPending}
                                        ...
                                        {isEditing && (
                                          <div className="flex items-center gap-2.5">
                                            <Checkbox
                                              ...
                                              onCheckedChange={handleModuleCheckAll(role.id, moduleKey)}
                                              disabled={updatePermissionsMutation.isPending}
                                            ...
                                            >
                                              {isEditing ? (
                                                <Checkbox
                                                  ...
                                                  checked={isChecked}
                                                  onCheckedChange={handlePermissionCheck(role.id, perm.id)}
                                                  disabled={updatePermissionsMutation.isPending}
                                                  className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                                                />
                                              ) : (
