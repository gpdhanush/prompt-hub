...
// Memoized Reimbursement Row Component
const ReimbursementRow = React.memo(({ 
  reimbursement,
  currencySymbol,
  canApprove,
  onApprove,
  ...
  const canApproveThis = canApprove(reimbursement);
    ...
    >
      <TableCell>
        <p className="font-bold text-sm text-primary">
          {reimbursement.claim_code || `CLM-${reimbursement.id}`}
        </p>
      </TableCell>
      <TableCell>
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-medium text-sm">{reimbursement.employee_name || 'N/A'}</p>
            <p className="text-xs text-muted-foreground">{reimbursement.emp_code || ''}</p>
          </div>
        </div>
      </TableCell>
      <TableCell className="font-semibold">{formatCurrency(parseFloat(reimbursement.amount || 0))}</TableCell>
      <TableCell>
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Calendar className="h-3.5 w-3.5" />
          {new Date(reimbursement.submitted_at || reimbursement.created_at).toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric"
          })}
        ...
        <div className="flex flex-col gap-1">
          ...
          {reimbursement.current_approval_level === 'Level 2' && reimbursement.pending_level_1_manager_name && (
            <p className="text-xs text-muted-foreground">
              Waiting for: {reimbursement.pending_level_1_manager_name}
            </p>
          )}
      ...
      <TableCell>
        <StatusBadge variant={getStatusVariant(reimbursement.status) as "error" | "success" | "info" | "warning" | "neutral" | "purple"}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={(e) => {
                e.stopPropagation();
                onView(reimbursement.id);
              }}>
                <Eye className="mr-2 h-4 w-4" />
                View Details
              </DropdownMenuItem>
              {reimbursement.attachment_count > 0 && (
                <DropdownMenuItem onClick={(e) => {
                  e.stopPropagation();
                  onView(reimbursement.id);
                }}>
                  <FileText className="mr-2 h-4 w-4" />
                  View Files ({reimbursement.attachment_count})
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
...
ReimbursementRow.displayName = 'ReimbursementRow';
...
export default function Reimbursements() {
  ...
  const canCreate = useMemo(() => isSuperAdmin || hasPermission('reimbursements.create'), [isSuperAdmin, hasPermission]);
  const canApproveReject = useMemo(() => isSuperAdmin || hasPermission('reimbursements.approve'), [isSuperAdmin, hasPermission]);
  ...
  // Fetch reimbursements - optimized query
  ...
  // Memoized format currency helper
  const formatCurrency = useCallback((amount: number) => {
    return `${currencySymbol}${amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  }, [currencySymbol]);
  ...
  // Memoized check if user can approve/reject
  const canApprove = useCallback((reimbursement: any) => {
    ...
    return false;
  }, [canApproveReject, isSuperAdmin, isLevel1]);

  // Memoized get status variant
  ...
  const handleReject = useCallback((reimbursement: any) => {
  ...
  }, [rejectionReason, selectedReimbursement, rejectMutation]);
