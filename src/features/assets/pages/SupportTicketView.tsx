...
export default function SupportTicketView() {
  ...
  // Fetch ticket details - optimized query
  ...
  // Fetch comments - optimized query
  const { data: commentsData, isLoading: commentsLoading } = useQuery({
    queryKey: ['support-ticket-comments', ticketId],
    queryFn: () => assetsApi.getTicketComments(ticketId),
    enabled: ticketId > 0,
    staleTime: 1000 * 60 * 2, // 2 minutes (comments may update more frequently)
    gcTime: 1000 * 60 * 10, // 10 minutes
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
  });

  // Fetch attachments - optimized query
  ...
  const ticket = useMemo(() => ticketData?.data, [ticketData?.data]);
  ...
  const isTicketOwner = useMemo(() => 
    ticket && currentUser && ticket.employee_user_id === currentUser.id,
    [ticket, currentUser]
  );
  ...
  }, [comment, isInternal, addCommentMutation, validateInput]);
  ...
  }, [selectedFile, uploadAttachmentMutation]);

  const handleViewFile = useCallback((attachment: any) => {
  ...
  const handleDownloadFile = useCallback((attachment: any) => {
    const baseUrl = API_CONFIG.SERVER_URL || 'http://localhost:3001';
    const url = attachment.file_url?.startsWith('http')
      ? attachment.file_url
      : `${baseUrl}${attachment.file_url}`;
  ...
  // Memoized handlers for mutations
  ...
  // Memoized handlers for attachment actions (used in map)
  ...
  const handleDeleteAttachmentById = useCallback((attachmentId: number) => {
  ...
  const handleFileCommentChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
                      ...
                      <div className="flex gap-1 shrink-0">
                        ...
                        {(isAdmin || attachment.uploaded_by === currentUser?.id) && (
                          <Button
                            ...
                            onClick={() => handleDeleteAttachmentById(attachment.id)}
