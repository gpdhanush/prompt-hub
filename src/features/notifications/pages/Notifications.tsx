...
export default function Notifications() {
  ...
  // Fetch notifications - optimized query
  const { data: notificationsData, isLoading, error, refetch } = useQuery({
    ...
    refetchOnReconnect: true, // Keep this true for notifications
  });

  const notifications = notificationsData?.data || [];

  // Fetch unread count - optimized query
  const { data: unreadCountData } = useQuery({
    queryKey: ['notifications-unread-count'],
    queryFn: () => notificationsApi.getUnreadCount(),
    refetchInterval: 30000, // Refetch every 30 seconds
    staleTime: 0, // Always consider stale for unread count
  ...
  const stats = useMemo(() => {
    return {
      ...
      taskNotifications: notifications.filter((n: any) => n.type?.includes('task')).length,
    };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="glass-card">
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-status-error">{stats.unread}</div>
            <p className="text-xs text-muted-foreground">Unread</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-status-success">{stats.read}</div>
            <p className="text-xs text-muted-foreground">Read</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{stats.taskNotifications}</div>
