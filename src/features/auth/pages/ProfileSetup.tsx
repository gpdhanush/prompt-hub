...
export default function ProfileSetup() {
  ...
  // Fetch current user data - optimized query
  ...
  // Fetch employee data if exists - optimized query
  ...
  // Fetch documents if employee exists - optimized query
  ...
  // Date formatting helpers - memoized
  const formatDateFromDB = useCallback((dateValue: string | null | undefined): string => {
    if (!dateValue) return "";
  ...
  const formatDateForDB = useCallback((dateValue: string | null | undefined): string | null => {
  ...
  }, [userData, employeeData, formatDateFromDB]);
  ...
  }, [uploadFile, uploadDocumentType, uploadDocumentNumber, employeeData, uploadDocumentMutation]);

  const handleDeleteClick = useCallback((docId: number) => {
  ...
  }, [deleteDocId, deleteDocumentMutation]);
  ...
  // Validation helper functions - memoized
  ...
  const validateMobile = useCallback((mobile: string): string | null => {
  ...
  const validatePincode = useCallback((pincode: string): string | null => {
  ...
  const validateEmergencyContactNumber = useCallback((number: string): string | null => {
  ...
  const validateDateOfJoining = useCallback((date: string): string | null => {
    if (!date) {
  ...
  }, [formData, employeeData, validateName, validateEmail, validateMobile, validateDateOfBirth, validateGender, validateDateOfJoining, formatDateForDB, updateProfileMutation]);

  // Memoized available document types
