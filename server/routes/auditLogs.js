...
router.get('/', authorize('Super Admin', 'Admin'), async (req, res) => {
  try {
    ...
    if (search && search !== 'undefined' && search.trim() !== '') {
      whereConditions.push(`(u.name LIKE ? OR al.module LIKE ? OR al.action LIKE ?)`);
      const searchParam = `%${search}%`;
...
router.get('/filters/options', authorize('Super Admin', 'Admin'), async (req, res) => {
  try {
...
router.post('/:id/restore', authorize('Super Admin', 'Admin'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const [existing] = await db.query(`SELECT * FROM ${tableName} WHERE id = ?`, [itemId]);
    
    // Build UPDATE query dynamically based on before_data
    const fields = Object.keys(beforeData).filter(key => key !== 'id' && key !== 'created_at');
    ...
    if (existing.length === 0 && auditLog.action === 'DELETE') {
      ...
      return res.status(400).json({ 
        error: 'Item was deleted. Full restoration from DELETE requires manual intervention.' 
      });
    }
...
// Export audit logs
router.get('/export/csv', authorize('Super Admin', 'Admin'), async (req, res) => {
  try {
    ...
    // Convert to CSV
    const csvHeader = 'ID,User,Action,Module,Item ID,Item Type,IP Address,Timestamp\n';
    const csvRows = logs.map(log => 
      `${log.id},"${log.user_name || 'N/A'}",${log.action},${log.module},${log.item_id || 'N/A'},${log.item_type || 'N/A'},"${log.ip_address || 'N/A'}",${log.created_at}`
    ).join('\n');
